...
// ============================================================================
// POST /api/company-hr/supervisors/bulk
// ----------------------------------------------------------------------------
// CSV BULK SITE SUPERVISOR IMPORT — Company HR only.
//
// Same shape as /api/program-coordinator/supervisors/bulk but for the
// Company HR → site supervisor workflow.
//
// SECURITY MODEL — nothing client-supplied is trusted for authorization:
//   - Caller role is read from the profiles table (DB truth).
//   - company_id is FORCED from the caller's own profile. Any
//     company_id column in the CSV body is IGNORED.
//   - Site supervisors are company-scoped — no program_ids needed.
//
// TWO-PHASE IMPORT:
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = rateLimiter.check(`hr-bulk-supervisors:${ip}`, RATE_LIMITS.general);
    if (!rl.allowed) {
    ...
    // ==========================================================
    // 2. DB-verified role + scope. ONLY company_hr may call.
    //    company_id is taken from the caller's profile.
    // ==========================================================
    ...
    if (profileErr || !callerRole || callerRole !== "company_hr") {
      return NextResponse.json<ApiResponse<never>>(
        { success: false, error: "Forbidden: Company HR access required" },
        { status: 403 }
      );
    }

    const { company_id: hrCompanyId } = callerProfile || {};
      ...
      return NextResponse.json<ApiResponse<never>>(
        {
          success: false,
          error:
            "Your account must be linked to a company before importing site supervisors.",
        },
        { status: 403 }
      );
    }
    ...
    // ==========================================================
    // 4. Parse request body: { csv, password?, dry_run }.
    //    NOTE: company_id in the body or CSV is deliberately
    //    IGNORED — it comes from the caller's profile.
    // ==========================================================
    ...
    // ==========================================================
    // 5. VALIDATE every row (dry run AND commit both run this).
    // ==========================================================
    ...
    // ==========================================================
    // 6. COMMIT: create accounts for valid rows only. Per-row
    ...
    for (const report of validRows) {
      ...
      // Site supervisors are company-scoped — no program_ids.
    ...
    return NextResponse.json({
      ...
      message: `Imported ${createdCount} site supervisor(s) into your company, ${inputs.length - createdCount} row(s) skipped.`,
    });
  } catch (error) {
    console.error("Error in POST /api/company-hr/supervisors/bulk:", error);
    return NextResponse.json<ApiResponse<never>>(
